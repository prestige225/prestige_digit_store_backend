...
def add_avis():
    ...
    note = data.get('note')
    commentaire = data.get('commentaire')
    product_id = data.get('product_id')  # <- nouveau champ

    if not nom or not note or not commentaire or not product_id:
        return jsonify({'message': 'Champs manquants'}), 400

    cursor = mysql.connection.cursor()
    try:
        cursor.execute("""
            INSERT INTO avis (nom, note, commentaire, date_post, product_id)
            VALUES (%s, %s, %s, NOW(), %s)
        """, (nom, note, commentaire, product_id))
        mysql.connection.commit()
        return jsonify({'message': 'Avis ajouté avec succès'}), 201
    except Exception as e:
        print("❌ Erreur insertion avis :", e)
