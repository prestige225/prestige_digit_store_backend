...
@app.route('/api/messages/sender', methods=['POST'])
def send_message():
    data = request.get_json()
    ...
    # Exemple avec une table "messages" contenant : id, name, email, message, sender, created_at
    cursor = mysql.connection.cursor()
    cursor.execute("""
        INSERT INTO messages (name, email, message, sender, created_at)
        VALUES (%s, %s, %s, %s, NOW())
    """, (name, email, message, sender))
    mysql.connection.commit()
    cursor.close()

    return jsonify({'success': True, 'message': 'Message envoyé'}), 200
