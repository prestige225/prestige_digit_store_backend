...
def commander():
    ...
    produits = data.get('produits')
    total = float(data.get('total'))  # 👈 assure-toi que c’est un float
    reduction_percent = int(data.get('reduction_percent', 0))  # 👈 par défaut 0%
    nom = data.get('client_nom')
    email = data.get('client_email')
    telephone = data.get('client_telephone')
    methode_paiement = data.get('methode_paiement')
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO commande (
                produits, total, client_nom, client_email,
                client_telephone, methode_paiement,
                client_adress_de_livraison, user_id, reduction_percent
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (
            json.dumps(produits_formates),
            total_apres_reduction,  # 👈 c’est ce total qu’on enregistre
            nom, email,
            telephone, methode_paiement, adresse,
            user_id, reduction_percent
        ))

        mysql.connection.commit()
        print("✅ Commande enregistrée avec réduction :", produits_formates)

        return jsonify({
            'message': 'Commande enregistrée avec succès',
            'total_apres_reduction': total_apres_reduction,
            'reduction_appliquee': reduction_percent
        }), 201
